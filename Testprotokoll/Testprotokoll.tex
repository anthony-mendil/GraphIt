...
\subtitle{Black-Box-Test}  % Oder White-Box-Test
\author{Clement Phung, Bastian Rexhäuser, Nina Unterberg, \\ Philipp Jacky Mach, Jonah Jeager, Anthony Mendil}  % Nicht ganz korrekte Auflistung der Authoren, sieht aber besser aus als mit \and
\date{21. Februar 2019}  % Datum der Testdurchführung, NICHT \today
...
\end{center}
\newpage
Der einzige Unterschied zum Benutzeroberfläche im Bearbeiter-Modus ist der, dass es in der Seitenleiste anstelle der Vorlage-Regeln den Verlauf gibt:
\begin{center}
...
\hfill Test Nr.: \setlength{\fboxsep}{1pt}\fbox{1}\fbox{2}\fbox{3} %etc.

\section{Ersteller-Modus}

\subsectiont{Sphären hinzufügen an freien Stellen}{\dotfill\ok\leer\leer}
...
\subsectiont{Sphäre hinzufügen, sodass es zu einer Überschneidung mit einer anderen kommen würde}{\dotfill\ok\leer\leer}
...
\subsectiont{Knoten hinzufügen an freien Stellen in Sphären}{\dotfill\ok\leer\leer}
...
\subsectiont{Knoten hinzufügen an belegter Stelle}{\dotfill\ok\leer\leer}
...
\subsectiont{Knoten hinzufügen außerhalb einer Sphäre}{\dotfill\ok\leer\leer}
...
\subsectiont{Verstärkende Relation zwischen zwei Symptome der selben Sphäre}{\dotfill\ok\leer\leer}
...
\subsectiont{Rückgängig machen (Undo)}{\dotfill\ok\leer\leer}
...
\subsectiont{Wiederherstellen (Redo)}{\dotfill\ok\leer\leer}
...
\subsectiont{Relation zwischen zwei Symptomen, zwischen denen bereits eine Relation existiert}{\dotfill\ok\leer\leer}
...
\subsectiont{Abschwächende Relation zwischen zwei Symptomen unterschiedlicher Sphären}{\dotfill\ok\leer\leer}
...
\subsectiont{Unbekannte Relation zwischen zwei Symptomen\\ unterschiedlicher Sphären}{\dotfill\ok\leer\leer}
...
\subsectiont{Schließen des Programms ohne den Graphen zu speichern}{\dotfill\ok\leer\leer}
...
\subsectiont{Verstoß gegen die eigenen Vorlage-Regeln}{\dotfill\ok\leer\leer}
...
\subsectiont{Speichern der Datei in den eigenen Dateien}{\dotfill\ok\leer\leer}
...
\subsectiont{Keinen Verlauf im Ersteller-Modus erstellen}{\dotfill\ok\leer\leer}
...
\subsectiont{Mehr Sphären hinzufügen als in den Vorlage-Regeln erlaubt}{\dotfill\ok\leer\leer}
...
\subsectiont{Abschwächende Relation hinzufügen obwohl es in den Vorlage-Regeln verboten wird}{\dotfill\ok\leer\leer}
...
\subsectiont{Einer Sphäre ein Symptom hinzufügen obwohl dies durch die Vorlage-Regeln verboten wird}{\dotfill\ok\leer\leer}
...
\subsectiont{Titel einer Sphäre ändern bei der es in den Vorlage-Regeln verboten wird}{\dotfill\ok\leer\leer}
...
\subsectiont{Style eines Symptoms ändern bei dem es in den Vorlage-Regeln verboten wird}{\dotfill\ok\leer\leer}
...
\subsectiont{Relationsart einer Relation ändern bei der es in den Vorlage-Regeln verboten wird}{\dotfill\ok\leer\leer}
...
\subsectiont{Entfernen einer Sphäre mit Symptomen und Relationen}{\dotfill\ok\leer\leer}
...
\subsectiont{Vergrößern einer Sphäre}{\dotfill\ok\leer\leer}
...
\subsectiont{Verkleinern einer Sphäre}{\dotfill\ok\leer\leer}
...
\subsectiont{Ändern der Farbe einer Sphäre}{\dotfill\ok\leer\leer}
...
\subsectiont{Ändern der Schriftart einer Sphäre}{\dotfill\ok\leer\leer}
...
\subsectiont{Verändern der Schriftgröße einer Sphäre}{\dotfill\ok\leer\leer}
...
\subsectiont{Sphären automatisch anordnen}{\dotfill\ok\leer\leer}
...
\subsectiont{Rückgängig machen der automatischen Anordnung von Sphären}{\dotfill\ok\leer\leer}
...
\subsectiont{Bewegen einer Sphäre auf eine andere}{\dotfill\ok\leer\leer}
...
\subsectiont{Entfernen eines Symptoms, das Relationen hat}{\dotfill\ok\leer\leer}
...
\subsectiont{Vergrößern eines Symptoms}{\dotfill\ok\leer\leer}
...
\subsectiont{Verkleinern eines Symptoms}{\dotfill\ok\leer\leer}
...
\subsectiont{Ändern der Füllfarbe eines Symptoms}{\dotfill\ok\leer\leer}
...
\subsectiont{Ändern der Umrandungsfarbe eines Symptoms}{\dotfill\ok\leer\leer}
...
\subsectiont{Ändern der Form eines Symptoms}{\dotfill\ok\leer\leer}
...
\subsectiont{Automatisches anordnen der Knoten}{\dotfill\ok\leer\leer}
...
\subsectiont{Ändern der Schriftart mehrerer Symptome, wobei dies durch die Vorlage-Regeln nicht bei allen erlaubt wird}{\dotfill\ok\leer\leer}
...
\subsectiont{Ändern der Schriftgröße eines Symptoms}{\dotfill\ok\leer\leer}
...
\subsectiont{Bewegen eines Symptoms in eine andere Sphäre}{\dotfill\ok\leer\leer}
...
\subsectiont{Bewegen eines Symptoms an eine Stelle \\an der keine Sphäre liegt}{\dotfill\ok\leer\leer}
...
Für die Tests für den Verlauf und die Übersicht erstellen wir vorerst mit \texttt{Datei} und \texttt{Neue Datei} eine neue Datei. Anschließend führen wir folgende Aktionen aus: (Positionen und Verschiebungen, sowie Farben können bei jedem Test variieren)\\
\begin{itemize}  
...
\subsectiont{Exportieren als Protokoll}{\dotfill\ok\leer\leer}
...
\subsectiont{Anzeige des Verlaufs innerhalb der Applikation}{\dotfill\ok\leer\leer}
...
\subsectiont{Verlauf filtern}{\dotfill\ok\leer\leer}
...
\subsectiont{Graph ändern und Verlauf neu laden}{\dotfill\ok\leer\leer}
...
\subsectiont{Anzeige der kompletten Übersicht}{\dotfill\ok\leer\leer}
...
\subsectiont{Relation in der Übersicht auswählen}{\dotfill\ok\leer\leer}
...
\subsectiont{Anzeigen ausschließlich verstärkender Relationen}{\dotfill\ok\leer\leer}
...
\subsectiont{Textsuche}{\dotfill\ok\leer\leer}
...
\subsectiont{Zoom Auswahl}{\dotfill\ok\leer\leer}
...
\subsectiont{Zoom Bar}{\dotfill\ok\leer\leer}
...
\subsectiont{Zoom Kontext}{\dotfill\ok\leer\leer}
...
\subsectiont{Ändern der Relationsart einer Relation über die Übersicht}{\dotfill\ok\leer\leer}
Vorerst wird oben in der Kopfleiste unter \texttt{Relation} verstärkende Relationen eingestellt. Anschließend macht der Bearbeiter einen Rechtsklick in der Übersicht auf die Relation von \texttt{Symptom 1} zu \texttt{Dinkelberry} und drückt auf Pfeilspitze. Hierbei wird erwartet, dass sich die Relationsart der Relation entsprechend ändert.
\subsubsection{Notizen}
...
\subsectiont{Änderung der Füllfarbe eines Symptoms über die Übersicht}{\dotfill\ok\leer\leer}
...
\subsectiont{Löschen einer Sphäre über die Übersicht}{\dotfill\ok\leer\leer}
...
Erklärung der Begriffe? (Philipp kannst du das vielleicht ergänzen?)

\subsectiont{Korrektes Berechnen und Anzeigen des Graphmaß, des Vernetzungsindex und des Strukurindex}{\dotfill\ok\leer\leer}
...
\subsectiont{Vorgänger von einem Symptom}{\dotfill\ok\leer\leer}
...
\subsectiont{Nachfolger von einem Symptom}{\dotfill\ok\leer\leer}
...
\subsectiont{Nachfolger und Vorgänger eines Symptoms}{\dotfill\ok\leer\leer}
Der Bearbeiter aktiviert in der Kopfleiste wieder \texttt{Vorgänger}. Nun sind beide, \texttt{Vorgänger} und \texttt{Nachfolger}, aktiviert und im Textfeld steht \texttt{2}. Hierbei wird erwartet, dass sowohl alle Symptome, zu denen man über maximal zwei Relationen von dem ausgewählten Symptom kommt, als auch alle von denen man über maximal drei Relationen zu dem ausgewählten Symptom kommt, hervorgehoben werden. 
\subsubsection{Notizen}
...
\subsectiont{Kürzester Weg von einem Symptom zu \\einem anderen (Weg existiert)}{\dotfill\ok\leer\leer}
...
\subsectiont{Kürzester Weg von einem Symptom zu \\einem anderen (Weg existiert nicht)}{\dotfill\ok\leer\leer}
...
\subsectiont{Alle Wege von einem Symptom zu einem anderen}{\dotfill\ok\leer\leer}
...
\subsectiont{Pfeilketten}{\dotfill\ok\leer\leer}
...
\subsectiont{Konvergente Verzweigungen}{\dotfill\ok\leer\leer}
...
\subsectiont{Divergente Verzweigungen}{\dotfill\ok\leer\leer}
...
\subsectiont{Verzweigungen}{\dotfill\ok\leer\leer}
...
\subsectiont{Zyklen}{\dotfill\ok\leer\leer}
...
\subsectiont{Vorlage exportieren}{\dotfill\ok\leer\leer}
...
\subsectiont{Vorlage importieren}{\dotfill\ok\leer\leer}
...
\subsectiont{GXL exportieren}{\dotfill\ok\leer\leer}
...
\subsectiont{GXL importieren}{\dotfill\ok\leer\leer}
...
\subsectiont{Exportieren als PDF}{\dotfill\ok\leer\leer}
...
\subsectiont{Drucken}{\dotfill\ok\leer\leer}
...
\subsectiont{Sprache der Benutzeroberfläche ändern}{\dotfill\ok\leer\leer}
Der Teste geht auf Optionen, Sprache des Benutzeroberfläche und wählt englisch aus. Hierbei wird erwartet, dass die Sprache der Benutzeroberfläche auf englisch gestellt wird, die des Graphen jedoch deutsch bleibt.
\subsubsection{Notizen}
...
\subsectiont{Sprache des Graphen und der Benutzeroberfläche ändern}{\dotfill\ok\leer\leer}
