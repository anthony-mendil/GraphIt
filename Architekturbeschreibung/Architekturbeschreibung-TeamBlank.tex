...
Das Objekt der Klasse \texttt{ActionHistory} aus dem Package \texttt{actions} wird benötigt, um die Actions (gemäß Strategie \hyperlink{command}{S9} dem Entwurfsmuster Command folgend} mit den Interaktionen des Nutzers zu verknüpfen. Diese führt zuvor erstellte Actions aus. Außerdem kann so die benötigte \texttt{redo}- und \texttt{undo}-Funktionalität umgesetzt werden. \\
