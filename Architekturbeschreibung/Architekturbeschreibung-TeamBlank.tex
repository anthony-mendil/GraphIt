...
Nach der Strategie in “\hyperlink{yy}{Datensicherung / Persistenz des Graphen}” wird über einer control- und data-Schnittstelle mit der Handler-Komponente kommuniziert. Die GXL-Komponente stammt von der Bibliothek “SourceForge” und ist zuständig für das Importieren und Exportieren der GXL-Dateien. \\ \\
...
Da der Graph gemäß Strategie \hyperlink{yy}{S1 – GXL} als text/plain gespeichert werden soll, in dem eine GXL (Graph eXchange Language) –Repräsentation des Graphen steht, muss es ein Modul geben, das alle Informationen des in der GUI angezeigten Graphen abfragen und diese ins GXL-Format transferieren kann. Um diese Funktion zu realisieren haben wir im \texttt{io-package} die Klasse \texttt{GXLio} eingefügt. In dieser sind Methoden zum Erstellen einer GXL-Repräsentation eines Graphen zu finden sowie Methoden zum Ex- und Import und zum Schreiben einer GXL-Repräsentation (in Form eines Strings) in die Datenbank. Über eine Instanz der \texttt{Syndrom-Klasse}, die im \texttt{graph-package} des darüber liegenden \texttt{graph-packages} zu finden ist, erhält die \texttt{GXLio-Klasse} alle Informationen zu dem in der GUI angezeigten Graphen. Diese Informationen werden dann lediglich ins GXL-Format überführt. Durch diese Herangehensweise kann in der \texttt{GXLio-Klasse} nicht nur die GXL-Repräsentation erzeugt werden, die in die Datenbank geschrieben wird, sondern auch diejenige, die in eine GXL-Datei geschrieben werden soll. Dementsprechend wird in dieser Klasse auch der GXL-Export realisiert. Hierfür wird im Zuge der Implementierung der Methodenrümpfe das bei SourceForge gehostete GXL-Framework verwendet werden (siehe Strategie \hyperlink{xaa}{S56 Sourceforge Framework}. Entsprechend der Erzeugung einer GXL-Repräsentation aus einem \texttt{Syndrom-Objekt} für das Speichern und Exporieren eines Graphen wird beim Import aus einer GXL-Repräsentation ein neues \texttt{Syndrom-Objekt} erstellt. Durch das \texttt{Template-Attribut} in der \texttt{Syndrom-Klasse} haben Methoden zum Erstellen einer GXL-Repräsentation eines Graphen aus einem \texttt{Syndrom-Objekt} auch Zugriff auf die Bearbeitungsregeln. Denn diese können in einem Objekt der \texttt{Template-Klasse} hinterlegt werden. Durch diesen Ansatz können die Bearbeitungsregeln in einer GXL-Repräsentation gespeichert werden – so wie wir es in Strategie \hyperlink{vvv}{S61 - Regeln in der GXL-Datei} vorgesehen haben. \\ \\

Ebenfalls im \texttt{io-package} ist die OOF-Klasse zu finden. Mit dieser wollen wir die gemeinsame Speicherung der Nutzerinteraktionen mit einem (als GXL-Repräsentation vorliegendem) Graphen realisieren (so wie wir es in Startegie \hyperlink{ooo}{S43 - Eigener Datentyp} beschrieben haben). Um diesen gemeinsamen Datentyp zu erzeugen, gibt es Methoden, die eine GXL-Repräsentation eines Graphen und Nutzerinteraktionen in JSON erhalten, wobei diese beiden Parameter je als String übergeben werden. \\ \\

Im \texttt{io-package} ist darüber hinaus auch eine Klasse (\texttt{JSONio}) zu finden, deren Methoden den Umgang mit JSON-Objekten realisieren sollen. Die Funktionalität dieser Methoden soll später gemäß Strategie \hyperlink{xbb}{S63 - Jackson} realisiert werden. Die Tatsache, dass wir in unserem System mit JSON-Objekten arbeiten werden, ist darauf zurück zu führen, dass wir mit diesen gemäß Strategie \hypertarget{jjj}{S33 - JavaScript Object Notation (.json)} den Ex- und Import von Nutzerinteraktionen beziehungsweise der Parameter der Nutzerinteraktionen realisieren wollen. \\ \\

Mit den Methoden der \texttt{PDFio-Klasse}, die ebenfalls im \texttt{io-package} zu finden ist, werden später unter Einsatz der Frameworks beziehungsweise APIs der PDF-Druck (gemäß Strategie \hyperlink{ggg}{S26: Java API}, die PDF-Erstellung (gemäß Strategie \hyperlink{hhh}{S29 - java.awt.Graphics2D} und der PDF-Export (gemä0 Strategeie \hyperlink{iii}{S30 - Freeheb Vectorgraphics} realisiert. \\ \\

Im package \texttt{gui} sind die Klassen \texttt{Main} und \texttt{Controller} enthalten. Erstgenannte Klasse enthält eine \texttt{main-Methode} und eine Methode, die als Parameter ein Stage-Objekt enthält und so das Starten unserer Anwendung unterstützen können wird. Die Verwendung des Stage-Objektes von JavaFX im Rahmen dieser Klasse im \texttt{gui-package} ist angelehnt an Strategie \hyperlink{kkk}{S34 - GUI Framework JavaFx}. Die \texttt{Controller-Klasse} enthält Methoden, die später die Methoden der Klassen im \texttt{actions-packages} aufrufen werden. (So wird beispielsweise die \texttt{addSphere()} im fertigen System die \texttt{AddSphereLogAction} aufrufen. Dieser Weiterleitung beziehungsweise dieses Kommunikationsprinzip, bei dem der Nutzer durch Interaktion mit der GUI (View) die \texttt{Controller-Klasse} veranlasst, Methoden der Actions (Model) aufzurufen, die dann wiederum zur Erzeugung von Einträgen in der Datenbank (Persistence) führen, ist unsere Umsetzung des Design-Patterns Model-View-Controller. Für dieses Entwurfsmuster haben wir uns in Form der Strategie \hyperlink{bbb}{S8 - MVC} entschieden. In der beschriebenen Kommunikationskette deutet sich durch das Vorhandensein von Actions, die in unserer Software als Klassen in den Packages im \texttt{actions-package} zu finden sind, bereits an, dass wir neben Model-View.Controller auch noch parallel das Command-Design-Pattern umgesetzt haben (siehe Strategie \hyperlink{xcc}{S9 - Command}). Von den Command und Model-View-Conroller einmal abgesehen haben wir gemäß Strategie \hyperlink{xdd}{S10 - Modularisierung} unser System weitreichend modularisiert. Dies zeigt sich zum Beispiel an der Untergliederung des Packages \texttt{actions} und des im \texttt{log\_management-package} befindlichen Packages \texttt{parameters} in weitere Packages. \\ \\

Das Zusammenspiel von Im- und Export beziehungsweise von der Erstellung von GXL-, PDF- oder OOF-bezogenen-Aspekten und dem Entwurfsmuster Command besteht darin, dass es Klassen im \texttt{export\_graph-package}, das wiederum im \texttt{actions-package} liegt, gibt, die aus der \texttt{Controller-Klasse} im Package \texttt{gui} (gemäß MVC) aufgerufen werden. Die Methoden in den Klassen des texttt{export\_graph-packages} wiederum werden später die Methoden der Klassen im package texttt{io} aufrufen. \\
...
\emph{Autoren: Jacky Philipp Mach, Jonah Jaeger}\\ \\
...
\begin{tabular} {|p{16cm}|}
	...
	\\\hline
	\textbf{Vorbedingungen}: Das Programm ist geschlossen, es wurden noch kein Graph erstellt und keine Einstellungen gesetzt. \\
	\textit{Ziel}: Eine Vorlage für einen Syndromansatz erstellen.
	\\\hline
		\textbf{Regulärer Ablauf}: \begin{itemize}
		\itemsep-0.7em
		\item der Ersteller öffnet die Application und drück auf den Menüpunkt \textit{Datei}  $\rightarrow$ \textit{Neuen Graph erstellen}
		\item das System öffnet ein Pop-Up-Eingabefeld 
		\item der Ersteller gibt den Namen des Graphen an und klickt ok
		\item der Ersteller klickt auf den \textit{Sphäre hinzufügen}- Button
		\item der Ersteller klickt in die Arbeitsfläche 
		\item der Ersteller klickt auf den Text der Sphäre
		\item das System öffnet ein Pop-Up-Eingabefeld 
		\item der Ersteller gibt den Namen der Sphäre ein
		\item der Ersteller markiert die Sphäre
		\item der Ersteller klickt den \textit{Farbe ändern}- Button 
		\item das System öffnet einen Color- Picker 
		\item der Ersteller wählt eine Farbe
		\item der Ersteller klickt mit einem Rechtsklick auf die Sphäre 
		\item das System öffnet ein Menü
		\item der Ersteller klickt \textit{Sphäre Bezeichnung blockieren}
		\item der Ersteller klickt mit einem Rechtsklick auf die Sphäre 
		\item das System öffnet ein Menü
		\item der Ersteller klickt \textit{Sphäre Farbe blockieren}
		\item der Ersteller klickt den Button \textit{Undo}
		\item der Ersteller wählt \textit{Exportieren als} $\rightarrow$ \textit{Vorlage}
		\item das System öffnet ein Pop-up-Fenster 
		\item der Ersteller gibt einen Speicherpfad an und drück den Button \textit{Exportieren}
	\end{itemize}
	\\\hline
	\textbf{Varianten}: Der Ersteller könnte weitere Element zur der Vorlage hinzufügen und eine maximale Anzahl für die Elemente setzen. 
	\\\hline
	\textbf{Nachbedingung}: Der Ersteller hat eine Vorlage exportiert. Er könnte nun in den Bearbeiter- Modus wechseln und dort die Vorlage bearbeiten. 
	\\\hline
	\textbf{Fehler-/Ausnahmefälle}:  Der Ersteller vergisst die GXL zu exportieren  $\rightarrow$ beim erneuten Start kann der Ersteller sich seine letzten Projekte anzeigen lassen und sein letztes Projekt auswählen. 
	\\\hline
\end{tabular}
...
\begin{tabular} {|p{16cm}|}
	\hline
	\rowcolor{anw}\parbox{16cm}{\textbf{A2: Bearbeiter modifiziert Vorlage}} \\\hline
	\hline
	\textbf{Akteure}:  Bearbeiter
	\\\hline
	\textbf{Vorbedingungen}: Das Programm ist geschlossen, die Vorlage in Form einer GXL-Datei ist vom Computer aus zugreifbar. \\ 
	\textit{Ziel}: Vorlage modifizieren und als neuen Datentyp speichern.
	\\\hline
	\textbf{Regulärer Ablauf}:
\begin{itemize}
	\itemsep-0.7em
	\item der Bearbeiter öffnet die Applikation und drückt auf den Menüpunkt \textit{Datei} $\rightarrow$ \textit{GXL importieren}
	\item das System öffnet ein Pop-Up Fenster
	\item der Bearbeiter gibt den Speicherpfad der GXL-Vorlage an und klickt auf \textit{Ok}
	\item das System öffnet ein Syndromansatz 
	\item der Bearbeiter klickt mit einem Rechtsklick auf eine Sphäre
	\item das System öffnet ein Menü
	\item der Bearbeiter klickt auf \textit{Knoten hinzufügen}
	\item der Bearbeiter klickt auf die Arbeitsfläche 
	\item das System öffnet ein Pop-Up-Eingabefeld
	\item der Bearbeiter gibt einen Namen \textit{A} ein
	\item der Bearbeiter klickt auf den \textit{Knoten hinzufügen}-Button
	\item der Bearbeiter klickt auf die Arbeitsfläche
	\item das System öffnet ein Pop-Up-Eingabefeld
	\item der Bearbeiter gibt einen Namen \textit{B} ein
	\item der Bearbeiter markiert diesen Knoten
	\item der Bearbeiter klickt auf den Menüpunkt \textit{Form ändern} $\rightarrow$ \textit{Quadrat}
	\item der Bearbeiter klickt auf den Menüpunkt \textit{Schriftart ändern} $\rightarrow$ \textit{Calibri}
	\item der Bearbeiter klickt auf den Knoten \textit{A}, zieht die Maus über den Knoten \textit{B} und lässt dann die Maus los
	\item der Bearbeiter markiert die Kante
	\item der Bearbeiter klickt auf den Menüpunkt \textit{Kante ändern} $\rightarrow$ \textit{Gestrichelt}
	\item der Bearbeiter klickt auf den knoten \textit{B}, zieht die Maus über eine andere Sphäre und lässt dann die Maus los
	\item der Bearbeiter drückt auf den \textit{Redo}-Button
	\item der Bearbeiter wählt \textit{Exportieren als} $\rightarrow$ \textit{OOF}
	\item der Bearbeiter gibt einen Speicherpfad an und drück den Button \textit{exportieren}
\end{itemize}
	\\\hline
	\textbf{Varianten}: Der Bearbeiter könnte neben den Einschränkungen viele Elemente hinzufügen und dies am Ende auch noch als Vorlage speichern.  
	\\\hline
...
\begin{tabular} {|p{16cm}|}
	\hline
	\rowcolor{anw}\parbox{16cm}{\textbf{A3: Auswerter wertet Graph und Bearbeiter aus}} \\\hline
	\hline
	\textbf{Akteure}:  Auswerter
	\\\hline
	\textbf{Vorbedingungen}: Das Programm ist geschlossen, ein bearbeiteter Syndromansatz in Form einer OOF-Datei ist vom Computer aus zugreifbar. \\ 
	\textit{Ziel}: Syndromansatz und Bearbeiter auswerten.
	\\\hline
	\textbf{Regulärer Ablauf}:
\begin{itemize}
	\itemsep-0.7em
	\item der Auswerter öffnet die Applikation und drückt auf den Menüpunkt \textit{Datei} $\rightarrow$ \textit{OOF importieren}
	\item das System öffnet ein Pop-Up Fenster
	\item der Auswerter gibt den Speicherpfad der OOF-Vorlage an und klickt auf \textit{Ok}
	\item das System öffnet den Syndromansatz
	\item der Auswerter drückt auf den Menüpunkt \textit{Option} $\rightarrow$ \textit{Sprache ändern}$\rightarrow$ \textit{Englisch}
	\item der Auswerter klickt mit einem Rechtsklick auf eine Sphäre 
	\item das System öffnet ein Menü
	\item der Auswerter klickt auf \textit{Ausblenden}
	\item der Auswerter klickt auf unter Zoom auf den \textit{Plus}-Button
	\item der Auswerter markiert mehrere Knoten in einer Sphäre
	\item der Auswerter klickt mit einem Rechtsklick auf die Knoten 
	\item das System öffnet ein Menü
	\item der Auswerter klickt auf \textit{Ausblenden}
	\item der Auswerter klickt auf den Menüpunkt \textit{Relationtyp} $\rightarrow$ \textit{nur verstärkend}
	\item der Auswerter klickt unter Zoom auf den \textit{Minus}-Button
	\item der Auswerter gibt \textit{Strg-Z} in die Tastatur 
	\item der Auswerter gibt \textit{Strg-Y} in die Tastatur 
	\item der Auswerter klickt mit einem Rechtsklick auf einen Knoten
	\item das System öffnet ein Menü
	\item der Auswerter klickt auf \textit{hervorheben}
	\item der Auswerter drückt auf \textit{Indizes berechnen}
	\item der Auswerter klickt \textit{Bearbeiter auswerten}
\end{itemize}
	\\\hline
	\textbf{Varianten}: Der Auswerter könnte nach vielen Kriterien filtern und diese auch miteinander kombinieren. Ebenfalls kann man bevor man in den Auswertermodus gelangt den Syndromansatz modifizieren.
	\\\hline
	\textbf{Nachbedingung}: Der Auswerter hat den Graphen nun ausgewertet. Danach könnte der Auswerter in den Bearbeiter-Modus wechseln und den Syndromansatz weiter bearbeiten.
	\\\hline
	\textbf{Fehler-/Ausnahmefälle mit deren Nachbedingung}: Der Auswerter könnte unglücklicherweise wichtige Knoten ausgeblendet haben. Man kann unter dem Button \textit{alle wieder anzeigen} das Ausblenden umkehren oder einfach den \textit{undo}-Button verwenden.
	\\\hline
\end{tabular}\\ \\ \\
...
\begin{tabular} {|p{16cm}|}
	...
	\textbf{Akteure}:  Bearbeiter
	\\\hline
	\textbf{Vorbedingungen}: Das Programm ist geöffnet. Die GXL-Datei ist erfolgreich importiert.\\ 
	\textit{Ziel}: Syndromansatz durch Modifikation korrigieren.
	\\\hline
	\textbf{Regulärer Ablauf}:
\begin{itemize}
	\itemsep-0.7em
	\item der Bearbeiter markiert zwei Symptome in der selben Sphäre
	\item der Bearbeiter klickt auf die Knoten, zieht die Maus über eine andere Sphäre und lässt dann die Maus los
	\item der Bearbeiter markiert die ausgehenden Kanten der eben genannten Knoten
	\item der Bearbeiter klickt auf den Menüpunkt \textit{Löschen}
	\item der Bearbeiter klickt auf den Menüpunkt \textit{Relationstyp ändern} $\rightarrow$ \textit{Abschwächend} 
	\item der Bearbeiter klickt auf den Menüpunkt \textit{Kante hinzufügen}
	\item der Bearbeiter klickt auf einen Knoten, zieht die Maus über eine anderen Knoten und lässt dann die Maus los
	\item der Bearbeiter markiert drei Relationen 
	\item das Bearbeiter klickt mit einem Rechtsklick auf die Kanten
	\item das System öffnet ein Menü
	\item der Bearbeiter klickt auf \textit{Relationstyp ändern} $\rightarrow$ \textit{ungewiss}
	\item der Bearbeiter markiert mehrere Knoten
	\item der Bearbeiter klickt auf den Menüpunkt \textit{Knotenform ändern} $\rightarrow$ \textit{Ellipse}
	\item der Bearbeiter markiert eine Kante
	\item der Bearbeiter klickt auf den Menüpunkt \textit{Kantendicke ändern} $\rightarrow$ \textit{Mittel}
	\item der Bearbeiter klickt auf den Color-Picker $\rightarrow$ \textit{Rot}
	\item der Bearbeiter markiert eine andere Kante
	\item der Bearbeiter klickt auf die \textit{Entf}-Taste der Tastatur
	\item der Bearbeiter klickt auf den Menüpunkt \textit{Speichern}
\end{itemize}
	\\\hline
...
\subsection{Anwendungsfall: Ein Lehrer stellt das Programm ein und erstellt einen Graphen}

\begin{tabular} {|p{16cm}|}
	\hline
	\rowcolor{anw}\parbox{16cm}{\textbf{A5: Ersteller stellt Programm ein und erstellt einen Graphen}} \\\hline
	\hline
	\textbf{Akteure}: Ersteller
	\\\hline
	\textbf{Vorbedingungen}: Das Programm ist geöffnet.\\ 
	\textit{Ziel}: System personalisieren und Graph ausdrucken.
	\\\hline
	\textbf{Regulärer Ablauf}:
\begin{itemize}
	\itemsep-0.7em
	\item der Ersteller klickt auf den Menüpunkt \textit{Optionen} $\rightarrow$ \textit{Standardwerte festlegen}
	\item das System öffnet ein Menü
	\item der Ersteller klickt auf den Menüpunkt \textit{Schriftart} $\rightarrow$ \textit{Arial}
	\item der Ersteller klickt auf den Menüpunkt \textit{Erstellen: Anzahl Sphären} $\rightarrow$ \textit{6} 
	\item der Ersteller klickt auf den Menüpunkt \textit{Datei} $\rightarrow$ \textit{Neuen Graph erstellen}
	\item das System öffnet ein Pop-Up-Eingabefeld
	\item der Ersteller gibt den Namen des Graphen an und klickt \textit{Ok} 
	\item der Ersteller klickt auf den Menüpunkt \textit{exportieren als} $\rightarrow$ \textit{PDF}
	\item das System öffnet ein Pop-Up-Fenster
	\item der Ersteller gibt einen Speicherpfad an und drückt auf Button \textit{Exportieren}
	\item der Ersteller klickt auf den Menüpunkt \textit{Datei} $\rightarrow$ \textit{Speichern unter}
	\item das System öffnet ein Pop-Up-Fenster
	\item der Ersteller gibt einen Speicherpfad an und drück den Button \textit{Speichern}
	\item der Ersteller klickt auf den Menüpunkt \textit{Exportieren als} $\rightarrow$ \textit{Vorlage}
	\item das System öffnet ein Pop-Up-Fenster
	\item der Ersteller klickt auf den Menüpunkt \textit{Maximale Knoten} $\rightarrow$ \textit{35}
	\item der Ersteller gibt einen Speicherpfad an und drück den Button \textit{Exportieren}
	\item der Ersteller klickt auf den Menüpunkt \textit{Hilfe}
	\item das System öffnet ein Pop-Up-Fenster
\end{itemize}
	\\\hline
	\textbf{Varianten}: Der Ersteller kann sehr viele verschieden Standardwerte festlege und diese auch miteinander kombinieren.
	\\\hline
	\textbf{Nachbedingung}: Das Programm ist nun nach den Wünschen des Benutzers eingestellt. Der Graph ist im GXL-Format und auch als PDF gespeichert. Dieser könnte der Ersteller in ausgedruckter Variante vervielfältigen.
	\\\hline
	\textbf{Fehler-/Ausnahmefälle mit deren Nachbedingung}: Der Ersteller könnte sehr ungünstige Standardwerte ausgewählt haben. Sollte dies der Fall sein, so kann der Benutzer jederzeit die Standardwerte neu setzen.
	\\\hline
\end{tabular}\\ \\ \\
...
In diesem Kapitel wollen wir beschreiben wie sich ändernde Anforderungen auf unsere Architektur auswirken würden. Dabei gehen wir zunächst auf die Einflussfaktoren ein, deren Veränderlichkeit wir im Rahmen der globalen Analyse als eher hoch oder gar sehr hoch eingeschätzt haben. Allerdings haben wir keinen Einflussfaktor als sehr veränderlich eingestuft, sodass wir lediglich jene mit einer eher hohen Veränderlichkeit thematisieren werden. Bei den entsprechenden Einflussfaktoren werden wir beschreiben, welchen Einfluss eine potentielle Änderung des Einflussfaktors (beziehungsweise der Anforderung, die durch diesen Einflussfaktor repräsentiert werden) auf unsere Architektur hätte. Im Anschluss daran werden wir auf die Anforderungen aus dem Chinese Menue eingehen, deren Umsetzung in unserer Architektur aktuell nicht vorgesehen ist. Hier werden wir erklären wie sich die nachträgliche Umsetzung dieser Anforderungen unsere Architektur beeinflussen würde. \\ 
...
Den Einflussfaktor \hyperlink{vv}{Entwickler-Fähigkeiten} haben wir ebenfalls als eher veränderlich eingestuft, da wir durch eigenständiges Lernen und insbesondere durch Tipps unseres Tutors unsere Fähigkeiten ausbauen können. Ein Ausbau unserer Fähigkeiten hat allerdings nicht zwangsläufig Änderungen der Architektur zufolge. Allerdings können wir in Folge erhöhter Entwicklungsfähigkeiten Änderungen durch sich verändernde oder hinzukommende Anforderungen leichter umgesetzt werden. \\ 
