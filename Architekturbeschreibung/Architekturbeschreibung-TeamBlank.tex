...
\begin{tabular} {|p{16cm}|}
	...
	\\\hline
		...
		\item der Ersteller macht klickt den Button \textit{undo}
...
\begin{itemize}
	...
	\item das System öffnet ein Syndromansatz mit fünf Sphären mit der Beschriftung 1-5.
	\item der Bearbeiter klickt mit einem Rechtsklick auf die Sphäre mit der Beschriftung 1.
	\item das System öffnet ein Menü
	\item der Bearbeiter klickt auf \textit{Knoten hinzufügen}
	\item der Bearbeiter gibt als Text des Knoten \textit{B} ein
	\item der Bearbeiter klickt auf den \textit{Knoten hinzufügen}-Button
	\item der Bearbeiter klickt auf die Sphäre mit der Bezeichnung 2
	\item der Bearbeiter gibt als Text des Knoten \textit{B} ein
	\item der Bearbeiter markiert den Knoten \textit{B}
	...
	\item der Bearbeiter bewegt den knoten \textit{B} in die Sphäre 5
	\item der Bearbeiter drückt auf den \textit{redo}-Button
	\item der Bearbeiter drückt auf den den Menüpunkt \textit{Datei} $\rightarrow$ \textit{Speichern}
	\item der Bearbeiter wählt \textit{Exportieren als} $\rightarrow$ \textit{oof}
\end{itemize}
	\\\hline
	Varianten: mögliche Abweichungen vom regulären Ablauf, 
	z.\,B.\ Auslassen oder Wiederholen von Aktionen
	\\\hline
	Nachbedingung: Endzustand und dann mögliche Folgeaktionen
	\\\hline
	Fehler-/Ausnahmefälle mit deren Nachbedingung; z.\,B.\ wie 
	wird auf ungültige Eingaben reagiert?
	\\\hline
\end{tabular}\\ \\ \\
...
\begin{tabular} {|p{16cm}|}
	\hline
	\rowcolor{anw}\parbox{16cm}{\textbf{Eindeutige Nummer: Eindeutiger Name}} \\\hline
	\hline
	Akteure: welche externen Instanzen interagieren mit dem 
	System in diesem Anwendungsfall?  
	\\\hline
	Vorbedingungen: Ausgangszustand, der vor Beginn des
	Anwendungsfalls gelten muss -- hier sollte auch das Ziel des 
	Akteurs genannt werden
	\\\hline
	Regulärer Ablauf: Abfolge von Aktionen der Akteure und
...
\begin{tabular} {|p{16cm}|}
	...
	Akteurs genannt werden
	\\\hline
       {Regulärer Ablauf}: \begin{itemize}
	\itemsep-0.7em
	\item der Ersteller klickt den Menü-Punkt \textit{Datei} $\rightarrow$ \textit{Exportieren als} $\rightarrow$ \textit{PDF}
	\item das System öffnet ein Pop-up Fenster 
	\item der Ersteller wählt ein Dateiformat, gibt einen Pfad an und drück den Button \textit{exportieren}  
	\end{itemize}
	\\\hline
	Varianten: mögliche Abweichungen vom regulären Ablauf, 
	z.\,B.\ Auslassen oder Wiederholen von Aktionen
...
Die Anforderung O«Wegpunkte» ließe sich mit dem Framework JGraph realisieren. Dazu müssten wir den Java-spezifischen Teil (JrapX) dieses Frameworks in unser System integrieren. Da man Wegpunkte innerhalb dieses Frameworks auf Edge-Objekten hinzufügt, müssten wir in unserem System statt unserer \texttt{Edge}-Klasse diejenige von JGraphX verwenden. Da diese ihrerseits allerdings nicht alle von uns benötigten Informationen (in Form von Attributen) enthält, müssten wir weiterhin eine eigene \texttt{Edge}-Klasse in unserem System vorhalten. Diese müsste allerdings anders als bis jetzt die Edge-Klasse von JGraphX erweitern. Diese alternative beziehungsweise neue \texttt{Edge}-Klasse würde dann überall dort verwendet, wo aktuell unsere jetzige \texttt{Edge}-Klasse angewandt wird. Beispiele hierfür sind die Attribute \texttt{vv} und \texttt{layout} vom Typ \texttt{VisualizationViewer<Vertex, Edge>} und \texttt{Layout<Vertex, Edge>} in der \texttt{Syndom}-Klasse im \texttt{graph.graph}-Package. Gerade durch diese beiden Attribute würden wir unserem Graphen Kanten zugänglich machen, die die hinzukommende Funktionalität der Wegpunkte umsetzen würden. Zu diesen Änderungen müsste auch im \texttt{actions}-Package je eine Klasse zum Hinzufügen beziehungsweise Entfernen von Wegpunkten hinzugefügt werden. \\ \\
