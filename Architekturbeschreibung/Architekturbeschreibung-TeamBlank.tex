...
Eine Übersicht über alle \texttt{Actions}, die es gibt, findet sich nach Paketen gegliedert in der folgenden Abbildung:
...
Zudem haben alle \texttt{LogActions} eine Verbindung zu \texttt{LogEntryName}. Dies ist für die Persistierung von Bedeutung. \\ \\
...
Die \texttt{Parameter}-Klassen finden sich im Paket \texttt{log\_management/parameters/activate\_deactivate}, \texttt{log\_management/parameters/add\_remove}, \texttt{log\_management/parameters/edit} und \texttt{log\_management/parameters/move}. Die Klassen haben individuelle Verbindungen zu Teilen des \texttt{graph/graph}-Packages. So ist beispielsweise die \texttt{EditFontSphereParam} mit \texttt{Shpere}, die \texttt{AddRemoveAnchorPointsParam} mit \texttt{Edge}, die \texttt{EditEdgesTypePram} mit \texttt{EdgeArrowType} und die \texttt{EditVerticesSizeParam} mit \texttt{Vertex} verbunden. Die Klassen innerhalb des Pakets \texttt{graph.graph} sind auf geeignete Weise miteinander verknüpft. \\ \\
