...
\includepdf[pages=-]{package.pdf}
\includepdf[pages=-]{graph_graph.pdf}
\includepdf[pages=-]{transformer.pdf}
\includepdf[pages=-]{predicates.pdf}
\includepdf[pages=-]{io.pdf}
\includepdf[pages=-]{jgrapht.pdf}
\includepdf[pages=-]{actions.pdf}
\includepdf[pages=-]{actionsOverview.pdf}
\includepdf[pages=-]{parameters.pdf}
\includepdf[pages=-]{log_management.pdf}
...
\begin{tabular} {|p{16cm}|}
	...
	\\\hline
	\textbf{Nachbedingung}: Der Ersteller hat eine Vorlage erstellt. Diese kann der Ersteller nun exportieren und als Vorlage nutzen.
	\\\hline
	\textbf{Fehler-/Ausnahmefälle}:  Der Ersteller vergisst am Ende Rahmenbedingungen zu setzen $\rightarrow$ Für den Fall, dass der Ersteller dies gespeichert hat, kann der Ersteller das Projekt neu importieren und dann die Regeln festlegen.
	\\\hline
\end{tabular}
...
\begin{tabular} {|p{16cm}|}
	...
	\\\hline
	\textbf{Vorbedingungen}: Das Programm ist geöffnet, die Vorlage in Form einer GXL-Datei ist schon importiert. \\ 
...
\begin{itemize}
	...
	\item der Auswerter klickt auf \textit{undo}
	\item der Auswerter klickt auf \textit{redo} 
...
\subsection{Anwendungsfall: Ein Lehrer importiert einen Graphen und exportiert diesen}

\begin{tabular} {|p{16cm}|}
	\hline
	\rowcolor{anw}\parbox{16cm}{\textbf{A5: Bearbeiter importiert einen Graphen und exportiert diesen}} \\\hline
	\hline
	\textbf{Akteure}: Bearbeiter
	\\\hline
	\textbf{Vorbedingungen}: Das Programm ist geöffnet.\\ 
	\textit{Ziel}: Graphen importieren und exportieren.
	\\\hline
	\textbf{Regulärer Ablauf}:
\begin{itemize}
	\itemsep-0.7em
	\item der Bearbeiter klickt auf den Menüpunkt \textit{Datei} $\rightarrow$ \textit{GXL-importieren}
	\item das System öffnet ein Pop-Up-Eingabefeld
	\item der Bearbeiter gibt den Speicherpfad des Graphen an und klickt \textit{Ok} 
	\item der Bearbeiter klickt auf den Menüpunkt \textit{exportieren als} $\rightarrow$ \textit{PDF}
	\item das System öffnet ein Pop-Up-Fenster
	\item der Bearbeiter gibt einen Speicherpfad an und drückt auf Button \textit{Exportieren}
	\item der Bearbeiter klickt auf den Menüpunkt \textit{Datei} $\rightarrow$ \textit{Speichern unter}
	\item das System öffnet ein Pop-Up-Fenster
	\item der Bearbeiter gibt einen Speicherpfad an und drück den Button \textit{Speichern}
	\item der Bearbeiter klickt auf den Menüpunkt \textit{Exportieren als} $\rightarrow$ \textit{Vorlage}
	\item das System öffnet ein Pop-Up-Fenster
	\item der Bearbeiter gibt einen Speicherpfad an und drück den Button \textit{Exportieren}
	\item der Bearbeiter klickt auf den Menüpunkt \textit{Hilfe}
	\item das System öffnet ein Pop-Up-Fenster
\end{itemize}
	\\\hline
	\textbf{Varianten}: Zwischendurch kann der Bearbeiter den Graphen modifizieren und anschließend exportieren, sodass der Bearbeiter viele Exporte verschiedener Versionen hat.
	\\\hline
	\textbf{Nachbedingung}: Der Graph ist nun importiert. Der Graph ist im GXL-Format und auch als PDF gespeichert. Dieser könnte der Ersteller in ausgedruckter Variante vervielfältigen.
	\\\hline
	\textbf{Fehler-/Ausnahmefälle mit deren Nachbedingung}: Der Bearbeiter könnte nun im Bearbeitermodus den Graphen modifizieren.
	\\\hline
\end{tabular}\\ \\ \\
