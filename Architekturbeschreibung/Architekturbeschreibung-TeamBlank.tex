...
In der folgenden Abbildung ist die konzeptionelle Sicht der Application visualisiert, mit all ihren Komponente und wie diese unter sich zusammenhängen und kommunizieren.\\
...
Dabei ist die GUI-Komponente die \textit{View}, die Command-Komponente die \textit{Controller} und die Datamodel-Komponente das \textit{Modell}. \\ 

In der \texttt{GUI}-Komponente wird hauptsächlich das Visuelle unserer Applikation festgelegt und wird wie in \hyperlink{kkk}{Benutzeroberfläche} festgelegt mit JavaFx umgesetzt. 
...
Dabei steht die \texttt{GUI}-Komponente mit dem Teilelement \texttt{Visualization} der Komponente \texttt{JUNG} in Beziehung über eine data-Schnittstelle. \\
...
Die Komponente \texttt{JUNG} wurde in die Teilkomponenten \texttt{Visualization}, \texttt{Algorithm} und \texttt{Graph} unterteilt. Das Framework JUNG besitzt selbst mehrere Komponenten und da unser System nur diese Komponenten integriert, haben wir uns an dieser Aufteilung orientiert.\\
Die \texttt{Algorithm}-Komponente ist für das Layout und Filtern des Graphen zuständig und benötigt deshalb Informationen von der \texttt{Graph}-Komponente. \texttt{Graph} beinhaltet hauptsächlich alle Datenklasse, die für die Graph Visualisierung benötigt werden und noch weitere die, die Funktionalität des Graphen implementieren.\\

In der \texttt{Command}-Komponente befindet sich die meiste Logik unserer Applikation. Für die Interaktion mit dem Graphen wird über eine control-Schnittstelle die Logik des Graphen angesprochen um die Editierung und Interaktion an diesem möglich zu machen. Die Logik des Graphen befindet sich dabei in der \texttt{Graph}-Komponente in JUNG. Zusätzlich besitzt Command auch die Undo/Redo-Funktionalität.\\
...
Gemäß der Strategie in \hyperlink{jjj}{Protokoll der Nutzerinteraktion exportieren/importieren} wird dies umgesetzt in der Komponente \texttt{Handler} mit Hilfe der \texttt{Jackson} Komponente. Die \texttt{Jackson} Komponente dient als Schnittstelle für das JSON Format und unterstützt das System die Parameter zu importieren und exportieren(\hyperlink{www}{Persistierung der Parameter in die DB als JSON-String}).\\
Auch legt die \texttt{Handler} Komponente fest, wie folgendes Problem \hyperlink{ooo}{Protokolldaten und Graphdaten zusammen sichern} umgesetzt wird und nutzt zur Hilfe die \texttt{Jackson} und \texttt{GXL} Komponente. \\
Für das Problem \hyperlink{hhh}{PDF Erstellung} nutzt das System die Komponente \texttt{FreeHEB Vectorgraphics} um java.awt.Graphics2D als zahlreiche Formate (z.B. SVG, PDF) zu exportieren, wie erwähnt in \hyperlink{iii}{Bibliothek für den PDF Export von java.awt.Graphics2D}. Dafür benötigen wir von der \texttt{JUNG} Komponente nur das Visualization-Element, welches über einer data-Schnittstelle übermittelt wird. Die Grafik wird der \texttt{Handler}-Komponente übergeben und diese setzt das Drucken(\hyperlink{ggg}{PDF Druck}) und Exportieren der PDF um. \\ 

Um den Graphen auswerten, benutzen das System die \texttt{Graph} Komponente in JUNG. Da diese Komponente jedoch nicht den benötigten Funktionsumfang deckt, haben wir in unsere Applikation auch die \texttt{JGraphT} Komponente eingebaut gemäß der beschriebenen Strategie in \hyperlink{fff}{Auswertung von Grapheigenschaften}. Dafür wird die Struktur des Graphen in der \texttt{JGraphT Handler} Komponente übersetzt, sodass JGraphT die Informationen zum Berechnen der Graph-Eigenschaften nutzen kann. \\ 

Die \texttt{Logger} Komponente ist in unserer Applikation eingebaut, um uns bei der Entwicklung und dem Debuggen zu unterstützen, indem sie die Applikation während der Laufzeit durch Logging aufzeichnet und uns zum Beispiel hilft Fehlerzustände oder das Verhalten der Applikation besser zu verstehen und nachzuvollziehen.\\ \\
\newpage
...
Über die injizierten Instanzen der \texttt{Syndrom}- und \texttt{GraphDao}-Klasse, erhalten die Objekte der \texttt{GXLio} Klasse alle Informationen zu dem in der GUI visualisierten Graphen. \\
Um diese Informationen in das GXL-Format zu überführen, wird im Zuge der Implementierung das GXL-Framework von SourceForge verwendet (siehe Strategie \hyperlink{ttt}{S56 Sourceforge Framework}).\\
...
In unserem Programm soll neben dem Editieren des Graphen auch das Auswerten des Graphen von großer Bedeutung sein. Für das Auswerten des Graphen müssen bestimmte Algorithmen zur Verfügung stehen.\\ Manche Algorithmen (z.B. Zyklensuche im Graphen) sind nach Strategie \hyperlink{xxx}{S25 - JGraphT} in dem Framework JGraphT implementiert, da sie nicht durch \textit{JUNG} zur Verfügung gestellt werden. Die Nutzung der Algorithmen lässt sich in dem Package \texttt{JGraphT} wiederfinden.\\
