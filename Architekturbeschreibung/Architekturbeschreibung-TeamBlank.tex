...
Alle dargestellten Attribute sind \texttt{private}, es existieren aber Getter und Setter für die Attribute (außer bei sämtlichen Id's, dort nur Getter). Deswegen sind die Attribute alle im Diagramm als \texttt{public} markiert. \\
Des weiteren lassen sich die Rollennamen direkt aus den Klassennamen herleiten, weshalb wir sie im Diagramm nicht explizit nennen. (Wir haben uns bezüglich der Klasse \texttt{Vertex} ... keine Ahnung...Hilfe) \\
...
Zum Starten und Beenden der Anwendung ist das Modul \texttt{gui} zuständig. Es arbeitet mit dem Modul \texttt{action} zusammen indem Programmteile aus \texttt{gui} (z.B. Buttons oder Menü-Items) bestimmte Actions aus dem Modul \texttt{actions} aufrufen. Durch den Aufruf dieser Actions kommt es zu Veränderungen des dargestellten Graphen (im Modul \texttt{graph}). Der geänderte Graph wird zusammen mit einem die Action beschreibenden Log-Eintrag in der Datenbank gespeichert. Dafür arbeitet das Modul \texttt{log\_management} mit dem Subsystem \texttt{Persistence} zusammen um das zweite Subsystem \texttt{H2DB} anzusteuern. Letzteres ist nur der Fall, falls die ausgeführte Action eine \texttt{LogAction} ist, welche für die Protokollierung des Nutzerverhaltens relevant ist.\\
Im selben Prozess werden Import und Export durch das Modul \texttt{io} geregelt. Diese werden ebenfalls durch Actions aus \texttt{actions} ausgelöst. \\
Zum Auswerten der Graph-Eigenschaften wird das Modul \texttt{jgrapht} benötigt. Auch diese Funktionen werden von den Actions in Kraft gesetzt.\\
...
Eine Alternative zu der bestehenden Version wäre die Realisierung der Anwendung durch mehrere Threads. Eine Möglichkeit wäre es, die GUI in einem eigenen Thread laufen zu lassen (siehe folgende Abbildung: Ausführungssicht der Multithread-Version). Dies könnte helfen, eine durchgehend ansprechbare und dadurch angenehme Benutzeroberfläche, gemäß \hyperlink{g}{Hoher Usability} und \hyperlink{h}{Hoher Qualität}, zu entwickeln. Falls unsere Architektur diese Aspekte nicht ausreichend abdeckt, was sich in Tests späterer Entwicklungsphasen zeigen wird, dann haben wir immer noch die möglich, auf dieses Modell umzusteigen. \\
