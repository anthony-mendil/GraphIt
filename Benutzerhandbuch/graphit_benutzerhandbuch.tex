...
Während der Nutzung des Programms kann es dazu kommen, dass sich ein Fenster öffnet, dass erst weggeklickt werden muss, bevor weitere Aktionen ausgeführt werden können. Diese Dialogfenster geben wichtige Hinweise für das Ergebnis der aktuellen (also zuletzt aufgerufenen Aktion). Sie sollten also unbedingt gelesen werden. Die Bestätigung des vorausgewählten Button (entwerder durch eienen Links-Klick auf den entsprecheneden Button oder durch das Drücken der \textit{Enter}-Taste können zu einem Ergebnis führen, dass nicht dem erwarteten Programmverhalten entspricht. Um das gewünschte Programmverhalten erzielen beziehungsweise das sich an die Bestätigung der gewünschten Aktion im Dialogfenster (durch einen Links-Klick auf den \textit{Weiter}-Button) anschließende Programmverhalten erklären zu können, gibt es in den Dialogfenstern immer (mindestens) zwei Möglichkeiten. Die Entscheidung für eine Möglichkeit schließt dabei die andere aus. Es können also nicht zeitgleich beide Möglichkeiten umgesetzt werden. Die angesprochenen Dialogfenster treten zum Beispiel auf, wenn eine neue Datei erstellt werden soll. In diesem Fall wird der in dem Dialogfenster darauf hingewiesen, dass der aktuell in der Anwendung angezeigte Graph durch Erstellung eines neuen Graphen überschrieben wird und somit nicht mehr auf diesen zurückgegriffen werden kann. Das Dialogfenster kann nun mit zwei möglichen Ausgängen geschlossen werden.
\begin{enumerate}
\item Durch einen Links-Klick auf \textit{Weiter} wird der aktuelle Graph mit einem neuen (vorerst leeeren) Graphen überschrieben,
\item Durch einen Links-Klick auf \textit{Abbrechen} wird kein neuer Graph erstellt, sodass der aktuell in der GUI angezeigte Graph weiter bearbeitet und exportiert werden kann.
\end{enumerate}
...
Beim File Chooser handelt es sich um das Fenster, dass die Auswahl eines Speicherorts ermöglicht. Dieses Fenster öffnet sich also stets, wenn es darum geht eine Datei zu speichern oder zu exportieren. In diesem Fall kann mit Hilfe des File Choosers die Stele im Dateisystem ausgewählt werden, an der die entsprechende Datei abgelegt werden soll. Bei diesen Aktionen kann zudem der Name angegeben werden, unter dem die entsprechende Datei abgespeichert werden soll. Neben den genannten Aktionen öffnet sich das Fenster mnit dem File Chooser auch, wenn eine existierende Datei geöffnet werden soll. In diesem Fall kann mit Hilfe des Fensters zu der Stelle im Dateisystem navigiert werden, an dem die zu importierende Datei gespeichert ist, und dort der entsprechende File ausgewählt werden. 
Die Navigation kann dabei je nach Präferenzen über ds Hauptfenster oder anhand der Seitenleiste erfolgen. Um die in einemOrdner enthaltenen Dateien im Hauptfenster angezeigt zu bekommen, kann der Ordner entweder mit einem Doppel-Klick im Hauptfenster oder einem einfachen Links-Klick in der Seitenleiste des Fensters ausgewählt werden. Um die Dateien eines Ordners in der Seitenleiste angezeigt zu bekommen, muss ein Doppelklick auf diesen Ordner in der Seitenleiste ausgeführt werden.
...
Das denke ich auch. Ich bekomme die Fehlermeldungen nicht angezeigt. Übernimm das bitte für mich :)
...
\subsection{Glossar}
\begin{description}
\item{Java 8} ist die Version der Programmiersprache Java, in der die Implementierung des vom Kunden geforderten Systems erfolgen wird. Java selbst ist eine objektorientierte Programmiersprache. \\ \\
...
\item{GXL} (Graph eXchange Language) ist ein Dateiformat, das dem Graphenaustausch zwischen Programmen dient. \\ \\
...
\item{.oof} ist ein von uns entwickeltes Dateiformat, das Nutzerinteraktionen und Graphinformationen zusammenführt. \\ \\
...
\item{\hypertarget{Symptom}{Symptom}} ist ein Knoten im Graphen.  \\ \\
...
\item{Ankerpunkte}  sind Stellen an einem Knoten, mit denen der Nutzer den Beginn oder das Ziel einer Kante mit gleichen Relationstypen definieren kann. \\ \\
