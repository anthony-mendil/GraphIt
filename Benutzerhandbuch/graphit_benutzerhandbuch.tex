...
\section{Instruktionen zur Nutzung des Programms} \label{sec:nutzung}
	...
	\condition
	Es muss ein Syndrom mit mindestens einem Element (eine Sphäre/ ein Symptom/ eine Relation), welches ausgewählt werden soll, existiert. 
	\actions
	Alternative 1: 
	\begin{enumerate}
	...
	\begin{itemize}
		...
		\item Die Nutzung der eben angesprochenen Übersichtsleiste wird im übernächstes Unterkapitel noch genauer beschrieben.
	\end{itemize}
	...
	%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
		...
		Die Übersichtsleiste befindet sich am linken Rand der Anwendung. Über diese Leiste kann ein Element des Syndroms sowohl ausgewählt als auch dessen Eigenschaften verändert werden. Des weiteren kann es als zusätzliche Hilfe herangezogen werden, um die Struktur des Syndroms zu untersuchen. So kann über diese Leiste (zusätzlich zur visuellen Darstellung des Syndroms) ermittelt werden, welche Sphäre, welche Symptome enthält und welche Symptome durch Relationen verbunden sind. In diesem Fenster ist auch ein Filter integriert, mit dem man die Beschriftung von Symptomen und die Relationstypen der Relationen filtern kann.

		\newpage
		%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
		\subsubsection{Navigation durch die Übersichtsleiste}		
	\condition
	Es muss ein Syndrom mit mindestens einer Sphären im Programm geöffnet sein, damit das Übersichtsfenster nicht leer ist. 
	\action
	\begin{enumerate}
		\item In der Übersichtsleiste den Titel der Sphäre, zu der man die ihr zugeordneten Symptome angezeigt bekommen möchte, mit einem Doppelklick auswählen. 
		\item Zum Schließen der Liste der Symptome dieser Sphäre einen erneuten Doppelklick auf den Titel dieser Sphäre ausführen oder einen Doppelklick auf den Titel des Symptoms ausführen, zu dem man die Liste der Relationen, die von ihm ausgehen, angezeigt bekommen möchte. 
		\item Durch einen Doppelklick auf den Titel eines Symptoms, zu dem die Liste der Relationen, die von diesem Symptom ausgehen, bereits angezeigt werden, kann die Liste wieder ausgeblendet / eingeklappt werden.	\end{enumerate}
	\hint
	\begin{itemize}
		\item Ist in der Übersichtsleiste kein Pfeil vor dem Titel einer Sphäre vorhanden, so enthält sie keine Symptome.
		\item Istin der Übersichtsleiste kein Pfeil vor dem Titel eines Symptoms vorhanden, so gehen keine Relationen von diesem Symptom aus.
	\end{itemize}
		 
				\newpage
		%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
				\subsubsection{Auswahl eines Graphelements über die Übersichtsleiste}		
	\condition
	Es muss ein Syndrom mit mindestens einer Sphären im Programm geöffnet sein, damit das Übersichtsfenster nicht leer ist. 
	\action
	\begin{enumerate}
	...
	\hint
	\begin{itemize}
		\item Das jeweils aktuell in der Übersichtsleiste ausgewählte Element wird auch in der (visuellen) Darstellung des Syndroms im Hauptfenster ausgewählt.
	\end{itemize}
		
				\newpage
		%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
		\subsubsection{Filtern (über die Übersichtsleiste)}
	\condition
	Es muss ein Syndrom mit mindestens einer Sphären im Programm geöffnet sein, damit das Übersichtsfenster nicht leer ist. 
	\action
	Filtern der Relationstypen:
	\begin{enumerate}
			\item In der Übersichtsleiste einen Links-Klick in die Check-Box neben dem \textit{Nach der Pfeilspitze der Relation filtern}-Button ausführen, sodass dort ein Haken erscheint.
		...
		\item In diesem Drop-Down-Menü, den Relationstyp, der gefiltert werden soll, mit einem Links-Klick auswählen. Hinweis: Der ausgewählte Relationstyp wird weiterhin im Graphen angezeigt. Die Relationen der anderen beiden Relationstypen werden ausgeblendet. Diese Ausblendung gilt sowohl für die visuelle Darstellung des Syndroms im Hauptfenster als auch für die Liste der Relationen in der Übersichtsleiste.
		...
		Filtern der Relationstypen:
		\begin{enumerate}
			\item In der Übersichtsleiste einen Links-Klick in die Check-Box neben dem \textit{Nach regulären Ausdrücken filtern}-Feld ausführen, sodass dort ein Haken erscheint.
		\item In der Übersichtsleiste einen Links-Klick in das Feld \textit{Nach regulären Ausdrücken filtern} klicken.
		\item Über die Computertastatur einen Suchbegriff eingeben, nach dem der Ttel von Symptomen gefiltert werden soll. Hinweis: Die Symptome, deren Titel den Suchbegriff enthalten,
	werden weiterhin im Graphen angezeigt. Die übrigen Symptome werden ausgeblendet. Diese Ausblendung gilt sowohl für die visuelle Darstellung des Syndroms im Hauptfenster als auch für die Liste der Symptome in der Übersichtsleiste.
		\item Um wieder alle Symptome einzublenden, erneut einen Links-Klick in die Check-Box neben dem \textit{Nach regüulären Ausdrücken filtern}-Feld ausführen, sodass der Haken in dieser Box verschwindet.
		\end{enumerate}
	\hint
	\begin{itemize}
		\item In beiden Filtervorgängen könne die ersten beiden Schritte auch in umgekehrter Reihenfolge durchgeführt werden.
		\item Die Filter können beliebig kombiniert werden.
		\item Das Ergebnis der Filterung schlägt sich sowohl in der Liste der Graphelemente in der Übersichtsleiste nieder als auch in der visuellen Darstellung des Syndroms im Hauptfenster.
	\end{itemize}		
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	\subsection{Editierung des Graphen - Allgemein}\label{edit}
		...
		\end{enumerate}			
				...
				\newpage
					%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
		\subsubsection{Tontextmenü öffnen}
		\condition 	
		...
		\begin{itemize}
				...
				\item Die Kontextmenüs von Sphären, Symptomen und Relationen unterscheiden sich.
				\item Das Kontextmenü lässt sich an zwei verschieden Stellen öffen: Zum einen im Hauptfenster, in dem der Graph visuell dargestellt wird, und zum anderen in der Übersichtsleiste. Im ersten Fall lässt es sich durch einen Rechts-Klick auf das dargestellte Graphelement und im Kontextmenü durch einen Rechts-Klick auf den Namen des entsprechenden Graphelements in der Übersichtsleiste.
	 	\end{itemize}	
